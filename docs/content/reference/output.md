...
describes what users get. Pick a format with `-o`, or let wheretheiss choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
wheretheiss <command> -o table   # aligned columns for reading
wheretheiss <command> -o jsonl   # one JSON object per line, for piping
wheretheiss <command> -o json    # a single JSON array
wheretheiss <command> -o csv     # spreadsheet friendly
wheretheiss <command> -o tsv     # tab-separated
wheretheiss <command> -o url     # just the URL column
wheretheiss <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
wheretheiss <command> --fields id,title,url
```
...
```bash
wheretheiss <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
wheretheiss <command>            # a table, because this is a terminal
wheretheiss <command> | wc -l    # JSONL, because this is a pipe
```
